  ...
  try {
    ...
    try {
      ...
      if (msg.includes('HTTP 429')) {
        ...
        return res.status(429).json({ res: false, error: 'ia_quota', message: 'La cuota diaria de la IA se agotó. Esperá unos minutos e intentá de nuevo, o pedile al admin que active el plan pago.' });
      }
      if (/HTTP 5\d\d/.test(msg) || msg.includes('timeout')) {
        res.set('Retry-After', '30');
        return res.status(503).json({ res: false, error: 'ia_busy', message: 'La IA está sobrecargada en este momento. Probá de nuevo en unos segundos.' });
      }
